...
有新信時直接通知手機。**不經過任何第三方帳號，也沒有額度限制**——你的電腦
把通知推給瀏覽器廠商的推播服務，內容是端對端加密的，中繼的 Apple／Google
伺服器看不到主旨。

### 前置條件

PWA 必須已經透過 `tailscale serve` 的 **https** 網址加到主畫面。
Web Push 需要安全來源；iOS 另外規定必須先加到主畫面才給開通知（iOS 16.4 以上）。

### 設定

```bash
npm run push:keys      # 產生金鑰，兩行貼進 .env
npm run serve          # 重啟
```

然後用手機打開主畫面上的 App，按右上角的 **開啟通知**，允許權限。
最後 `npm run notify:test` 確認收得到。

換掉金鑰的話，每台手機都要重新開啟一次通知。

### 通知內容

**只有寄件者和主旨**，跟 LINE 那邊一致。一次同步進來 3 封以內會分開通知，
超過就合併成一則「N 封新信」，免得一次跳十幾個。

### 跟 LINE 的取捨

| | Web Push | LINE |
| --- | --- | --- |
| 費用 | 免費無上限 | 按則計費，免費額度有限 |
| 內容給誰看 | 端對端加密，中繼者看不到 | 經過 LINE 的伺服器 |
| 需要什麼 | 已安裝的 PWA、iOS 16.4+ | LINE 帳號 |
| 電腦關機時 | 收不到 | 收不到 |

兩個可以同時開，也可以只開一個——`.env` 裡留空的那個就不會動。
...
`deploy/sync.cron` 讓 `npm run sync` 每 15 分鐘跑一次，加上推播就是完整的
自動通知：有新信 → 手機響 → 點進去讀。
...
- 通知只送寄件者和主旨。Web Push 的內容是端對端加密的；LINE 則會經過
  LINE 的伺服器，開了 `LINE_INCLUDE_SNIPPET` 連內文也會
- VAPID 私鑰放在 `.env`，不會進 git
